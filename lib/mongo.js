...
mongoOps.updateFields = async (collection, keys, updates, options = {}) => {
  let { db } = await connectToDatabase();

  let data = await db.collection(collection).updateMany(keys, updates, options);
  if (data.acknowledged) return data;
  return false;
};
